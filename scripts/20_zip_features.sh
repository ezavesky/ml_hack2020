...
if [ ! -f "$ROOT_DIR/dist/features_binary.zip" ]; then
    # see stack page https://stackoverflow.com/a/4269862 for why -- we want only leaf nodes!
    find . -type d -exec sh -c '(ls -p "{}"|grep />/dev/null)||echo "{}"' \; | grep -e '\(videocnn\|vggish\)' | xargs zip -9r $ROOT_DIR/dist/features_binary.zip
    # find ./ -type d | grep batches | grep -e '\(videocnn\|vggish\)' | xargs zip -9r $ROOT_DIR/dist/features_binary.zip
fi

# now get everything else
if [ ! -f "$ROOT_DIR/dist/features_tag.zip" ]; then
    find . -type d -exec sh -c '(ls -p "{}"|grep />/dev/null)||echo "{}"' \; | grep batches | grep -v -e '\(videocnn\|vggish\)' | xargs zip -9r $ROOT_DIR/dist/features_tag.zip
fi
